...
class Lookup:
    ...
    """
    def __init__(self, assembly_file, paths):
        self.assembly_file = assembly_file
        self.etc_folder = paths.etc_dir
        #
        self.mapping_attribute_dict = read_yaml(os.path.join(self.etc_folder, "attribute_mapper.yaml"))
        ...
        self.pragma_to_vcf_map = read_pragma_mapper(os.path.join(self.etc_folder, 'pragma_mapper.tsv'))
    ...
    def close(self):
        """Closes file handle."""
        if hasattr(self, 'assembly_fasta_indexed'):
            self.assembly_fasta_indexed.close()